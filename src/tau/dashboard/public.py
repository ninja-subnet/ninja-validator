...
def _public_duel_id(challenger_submission_id: object) -> int:
    return 100000 + (
        zlib.crc32(str(challenger_submission_id).encode("utf-8")) % 900000
    )
...
class DashboardConfig:
    ...
    def from_env(cls, environ: Mapping[str, str] | None = None) -> DashboardConfig:
        ...
        return cls(
            ...
            recent_duels=max(1, env_int(env, "TAU_DASHBOARD_RECENT_DUELS", d.recent_duels)),
            fresh_seconds=max(
...
class PublicDashboard:
    ...
    def response_for(self, path: str) -> tuple[int, dict[str, Any]]:
        ...
        if path == "/api/dashboard/pools":
            ...
            active_pool_name = active_duel.get("active_pool_name") if active_duel else None
            return HTTPStatus.OK, {
                "updated_at": payload["updated_at"],
                "active_pool": active_duel.get("pool") if active_duel else None,
                "active_pool_id": active_pool_id,
                "active_pool_name": active_pool_name,
                "active_pool_label": active_duel.get("pool_label") if active_duel else None,
        ...
        duels.append(
            {
                ...
                "king_score_mean": _float_or_none(row["king_score_mean"]),
                "challenger_score_mean": _float_or_none(row["challenger_score_mean"]),
                "score_mean_delta": _float_or_none(row["score_mean_delta"]),
                "mean_score_delta": _float_or_none(row["score_mean_delta"]),
                "final_mean_delta": _float_or_none(row["score_mean_delta"]),
                "score_mean_rounds": int(row["score_mean_rounds"] or 0),
                "score_notes_url": (
                    f"/api/dashboard/duels/{public_duel_id:06d}/notes"
                ),
                "king_uid": row["king_uid"],
                "king_hotkey": row["king_hotkey"],
                "king_repo": king_repo,
...
def _duel_score_notes(session: Session, *, duel_id: int) -> dict[str, Any] | None:
    ...
    return {
        ...
        "public_note": "Score-only duel notes. Task prompts, patches, solutions, judge rationales, model names, and task ids are intentionally omitted.",
...
def _worker_freshness(
    ...
    *,
    ...
    latest_task = session.execute(text("SELECT max(created_at) FROM tasks")).scalar_one()
    ...
    return {
        ...
        "solutions": _freshness_from_timestamp(
            ...
            detail=f"{solution_backlog} pending solution(s)" if solution_backlog else "",
        ),
...
def _freshness_from_timestamp(
    ...
    *,
    ...
    if value is None:
        return {"status": "unknown", "last_seen_at": None, "age_seconds": None, "detail": detail}
...
def _scoring_config(env: Mapping[str, str]) -> dict[str, Any]:
    ...
    if method not in {"round_wins", "mean"}:
        method = "round_wins"
    margin = env_float(env, "TAU_DUEL_MEAN_SCORE_MARGIN", 0.05)
